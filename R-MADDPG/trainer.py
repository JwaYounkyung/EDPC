...
def parse_args():
    ...
    parser.add_argument('--max-episodes', type=int, default=50_000, help='number of episodes')
    ...
    parser.add_argument('--noise-std', type=float, default=0.0, help='{0.0, 1.0, 2.0, 3.0, ...}, uncertainty level')
...
def make_env(scenario_name, shaped=True, noise_std=1.0, benchmark=False):
...
def log_writer(args, agents):
    ...
    if hasattr(agents[0].env.agents[0], 'adversary'):
        run_name = 'good_%s_adv_%s_noise_%.2f_' % (args.good_policy, args.adv_policy, args.noise_std)
    else:
        run_name = 'good_%s_noise_%.2f_' % (args.good_policy, args.noise_std)
...
def train(args):
    ...
    env = make_env(args.scenario, args.shaped, args.noise_std, args.benchmark)
...
def main():
    ...
    conf = 'conf:\n\tenv: %s\n\tgood vs adv: %s vs %s\n\tapprox actors: %r\n\tnoise_std: %.2f' % (
        args.scenario, args.good_policy, args.adv_policy, args.use_agent_models, args.noise_std)
    print(conf)
