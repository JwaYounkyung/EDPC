...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, nature_p_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        # create distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n]
        ...
        # R-MADDPG
        rew = nature_p_func(q_input, 1, scope="nature_p_func", num_units=num_units)[:,0]
#         target_ph += rew
